...
pub const AutoTune = struct {
    ...
    prev_phase: f32,
    pitch_hold_counter: usize, // Frames since last valid pitch detection
    ...
    pub fn process(self: *AutoTune, input: []const f32, output: []f32) void {
        ...
        // Hold time: 200ms at 48kHz with 1024 sample buffers = ~10 buffers
        const PITCH_HOLD_FRAMES: usize = 10;

        if (detected_freq > 0.0) {
            // Valid pitch detected - reset hold counter
            self.pitch_hold_counter = 0;
            ...
            // No pitch detected - increment hold counter
            self.pitch_hold_counter += 1;
        }

        // Apply pitch shifting if we have a valid pitch OR we're within the hold period
        if (self.detected_freq > 0.0 and self.pitch_hold_counter < PITCH_HOLD_FRAMES) {
            // 4. Apply pitch shift using PSOLA with current ratio
            ...
            // No valid pitch and hold expired - pass through unmodified
            if (output.ptr != input.ptr) @memcpy(output, input);
        }
    }
